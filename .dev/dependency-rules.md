...
| --- | ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------- | ------------------------------------------------------------------------------------- |
| 1   | Triage  | PR diff would touch `.github/`, CI workflows, `dependabot.yml`, or licensing files — **except** PRs from the `dependabot/github_actions/*` branch family where the diff is limited to `uses:` line changes in workflow files (those are mechanical version pins, not authored CI edits, and are eligible for Phase 2) | `ralphie:skip-out-of-scope`        | Maintainer scopes the change; remove label                                            |
| 2   | Phase 2 | Verification clean (lint/build/links) **and** changelog investigation finds no genuine concern                                                                                                                                                                                                                                  | `ralphie:ready-to-merge`           | Maintainer reads Ralphie's summary comment, clicks merge                              |
...
| 4   | Phase 2 | Upgrade requires multi-step migration that exceeds the loop's scope (framework majors with multi-major stepping; pre-1.0 → 1.0 jumps that touch a large surface)                                                                                                                                                                     | `ralphie:skip-manual-upgrade`      | Maintainer steps the upgrade manually; remove label                                   |
| 5   | Phase 2 | `pnpm lint` / `pnpm build` / `pnpm check:links` failed twice on causes the agent can't trace                                                                                                                                                                       | `ralphie:skip-verification-failed` | Read Ralphie's comment with cited failure output; fix locally                         |
...
- **Framework majors** — `next`, `react`, `react-dom`. Coordinate-the-whole-stack changes; even when `pnpm build` passes, the migration may have user-visible behavior shifts (RSC semantics, hydration timing, server actions) that don't show up in a static-export type check.
- **Build-tooling majors** — `@opennextjs/cloudflare`, `eslint-config-next`, `typescript`. These don't ship to users but a broken build blocks deploys and a misconfigured lint can hide real regressions. Read the migration notes.
- **Pre-1.0 → 1.0 jumps on runtime deps** — by definition the upstream is signaling "this is the stable shape." Even when v1.0 is API-compatible, bundle structure / ESM / module-resolution changes can affect Next.js' bundler.
- **Multi-major bumps** — e.g., a package jumping v0.546 → v1.14. Each major may have its own breaking changes; assess all of them.

For these categories, "doesn't affect us" requires more evidence than a quick grep. Read migration guides thoroughly, check all call-sites, run `pnpm build` and `pnpm check:links` and inspect the build output for warnings. Defaulting toward `skip-needs-review` (or `skip-manual-upgrade` when the migration would clearly exceed the loop's size cap) is correct here when the investigation is anything less than thorough.
...
- Path into the tree: <output of `pnpm why <pkg>`, fenced>.
...
- Never modify files under `.github/`, CI workflows, `dependabot.yml`, or licensing files. If the fixup would require any of these, apply `ralphie:skip-out-of-scope` and exit.
...
**Smallest fixup that lints + builds + passes the link check.** Don't refactor adjacent code. Don't take on tech-debt cleanup. Don't update unrelated callers. Surface drift via comments in the PR body, not by widening the diff.
