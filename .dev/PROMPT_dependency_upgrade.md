...
- **Build-tooling major** — `@opennextjs/cloudflare`, `eslint-config-next`, `typescript`, or anything else that runs during `pnpm build` / `pnpm lint`. A broken build blocks deploys.
- **Pre-1.0 → 1.0 jump on a runtime dep** (in `dependencies`, not `devDependencies`)
- **Multi-major bump** — e.g., a package jumping v0.546 → v1.14 across two majors
...
3. **Verify.** From the repo root:
   - `pnpm lint`
   - `pnpm build` — this runs Next.js' built-in type-check + bundle. A failed `pnpm build` is the docs equivalent of `tsc` + `next lint` + bundler error. Inspect the build output for warnings, not just exit code.
   - `pnpm check:links` — link-check across the rendered docs. Slow but catches breakage where a dep bump changed routing or rewrote anchors.
...
4. **Branch on the result:**
   ...
   **Path B.2 — lockfile-regeneration replacement (no behavioral change).**
   ...
   - **Before regenerating, inspect Dependabot's diff to detect the case shape:**
     ...
     **B.2.a — package.json + lockfile bumps (the typical case):**
     - Recipe: take package.json from Dependabot's branch and re-run install:
       ```
       git checkout <dependabot-branch-name> -- package.json
   ...
   - Verify everything is consistent: `git status` should show `package.json` and `pnpm-lock.yaml` modified — nothing else.
   - **Size gate** still applies: `git diff --stat origin/main...HEAD -- ':!package.json' ':!pnpm-lock.yaml' ':!*.md' ':!*.mdx'`. (Should be 0 changes — the regen path is supposed to be clean.) If somehow >10 files / >300 LoC, apply `ralphie:skip-too-big` with cited reason and exit.
   ...
   - Open replacement PR: `gh pr create --base main --assignee @me --title "chore: regenerate lockfile for <package or group> bump" --body "..."`. PR body MUST include:
     ...
     - Verification summary (lint/build/check:links output, all green).
   ...
   **Path B.3 — breaking-change fixups.**
   ...
   - Create a fresh branch: `git checkout -b chore/upgrade-<package>-v<major>` (e.g., `chore/upgrade-next-v16`).
   - Apply the version bump from the Dependabot branch:
     ```
     git checkout <dependabot-branch-name> -- package.json pnpm-lock.yaml
     pnpm install
     ```
...
**[2]** Hard constraints from @dependency-rules.md are non-negotiable: never push to `main`, never push to a Dependabot branch, never merge a Dependabot PR (the human is the gate, always), never modify `.github/`, CI workflows, `dependabot.yml`, or licensing files. If the fixup would require any of these, apply `ralphie:skip-out-of-scope` and exit.
...
**[9]** For elevated-scrutiny categories (framework majors, build-tooling majors, pre-1.0 → 1.0, multi-major), the burden of proof for `ready-to-merge` is higher. When in doubt between `ready-to-merge` and a skip, choose the skip — but the skip comment must surface what was already investigated, not just say "your turn."
