...
- `.github/workflows/deploy-production.yml` — on push to `main` (and manual `workflow_dispatch`), GitHub Actions auto-starts a production deploy that pauses at the `production` environment's approval gate. A required reviewer approves in the Actions UI, then the same `pnpm run deploy:production` runs in CI with `CLOUDFLARE_API_TOKEN` + `CLOUDFLARE_ACCOUNT_ID` from environment secrets.
